...
function BookingContent() {
  ...
  async function handlePayLater() {
    setLoading(true); setStep(3); setLoading(false)
  }
      ...
      <section className="container mx-auto mb-8 px-4">
        <div className="mx-auto max-w-2xl">
          ...
          <div className="mt-2 flex justify-between text-xs text-muted-foreground">
            <span>Select Slot</span><span>Payment</span><span>Confirmed</span>
          </div>
        </div>
      </section>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 font-serif">
                    <CalendarDays className="h-5 w-5 text-primary" />{t.booking.selectDate}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <Calendar mode="single" selected={selectedDate}
                    onSelect={(d) => { setSelectedDate(d); setSelectedTime('') }}
                    disabled={(date) => date < new Date()} className="rounded-2xl border" />
                </CardContent>
              </Card>
              ...
              <div className="space-y-4">
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2 font-serif">
                      <Clock className="h-5 w-5 text-primary" />{t.booking.selectTime}
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    {!selectedDate ? (
                      ...
                      <div className="flex items-center gap-2 text-muted-foreground">
                        <Loader2 className="h-4 w-4 animate-spin" /><span className="text-sm">Loading slots...</span>
                      </div>
                    ) : slots.length === 0 ? (
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {slots.map((slot) => (
                          <Button key={slot.time} variant={selectedTime === slot.time ? 'default' : 'outline'}
                            size="sm" disabled={!slot.available} onClick={() => setSelectedTime(slot.time)}
                ...
                <Card>
                  <CardHeader><CardTitle className="font-serif">Select Program</CardTitle></CardHeader>
                  <CardContent>
                    {programsLoading ? (
                      <div className="flex items-center gap-2 text-muted-foreground">
                      ...
                      <Select value={selectedProgram} onValueChange={setSelectedProgram}>
                        ...
                        <SelectContent>
                          {programs.map((p) => {
                            ...
                            return <SelectItem key={p._id} value={p._id}>{name} — €{p.price}</SelectItem>
                          })}
                        </SelectContent>
                      </Select>
                    )}
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <PayPalButtons
                    ...
                    createOrder={async () => {
                      const res = await fetch('/api/paypal/create-order', {
                        ...
                        body: JSON.stringify({
                          amount: selectedProgramData.price, bookingId,
                    ...
                    onApprove={async (data) => {
                      ...
                      if (result.success) { setMeetLink(result.meetLink || ''); setStep(3) }
                  ...
                  <p className="rounded-md bg-amber-50 dark:bg-amber-950/20 px-3 py-2 text-xs text-amber-700 dark:text-amber-400">
                    ⚠️ Payment must be completed at least 24 hours before your lesson.
                  </p>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <Button variant="ghost" onClick={handlePayLater} disabled={loading}
                      className="text-muted-foreground hover:text-foreground w-full">
                      <Clock3 className="mr-2 h-4 w-4" />I'll pay later
                    </Button>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="pt-8 pb-8 space-y-6">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      {meetLink
                        ? <>Payment received! Confirmation sent to <strong>{session?.user?.email}</strong></>
                        : <>Your lesson is booked! Pay later from your dashboard.</>}
                    </p>
                  </div>
                  {meetLink && (
